...
int pivotListY[n];
int pivotChoice;
int hor_count=0, ver_count=0, K_mer_counter=1;
int N_Kmer = ((rho*N)/K); 
int PBCX(int x);
int PBCY(int y);
...
void lat_init(){
 ...
 /*  i=0;
    ...
    while (i != N_Kmer); 
      ...
      for(i=0;i<Lx;i++){
         for(j=0; j<Ly; j+=K){
            if(K_mer_counter == N_Kmer+1){
               break;

         }
            for(k=j; k<(j+K); k++){
               Lat[i][k]= K_mer_counter;
...
#define K 7
#define N (Lx*Ly)
#define T 100000    //change
#define rho 0.8         //change
#define p 1
#define p1 1-p
#define p2 p/2.0
#define p3 p/2.0
...
int printLattice()
{
 /*  int remaining;
   ...
   if ( orientation_type[K_mer_counter]= h_orientation)          
                  {  
                     pivotListX[K_mer_counter] = pivotListX[pivotChoice];
                     headListY[K_mer_counter] = PBCY(pivotListY[pivotChoice]-(K/2));
                     if((headListY[K_mer_counter]+(K-1)) != PBCY(headListY[K_mer_counter]+(K-1))){
                        remaining = ((headListY[K_mer_counter]+(K-1))% Lx);
                        printf("\n K_mer_id %d %d %d 0 %d 1\n",(K_mer_counter+1), pivotListX[K_mer_counter], headListY[K_mer_counter], ((K-1) - remaining));
                        printf("\n K_mer_id %d %d 0 0 %d 1\n",(K_mer_counter+1),  pivotListX[K_mer_counter], remaining);
                     ...
                     headListX[K_mer_counter] = PBCX(pivotListX[pivotChoice] -(K/2));
...
{   
   ...
   for(t=0; t<T; t++)
   {
      ...
      if(t % GAP == 0){
      printf("time_step %llu  horizontal %d  vertical %d  ratio %f\n",t, hor_count, ver_count, ((1.0*ver_count)/(1.0*hor_count)));
      //printf("pvc%d\n",pivotChoice);
      //printf("pvcxy%d:%d\n",pivotListX[pivotChoice],pivotListY[pivotChoice]);
      //printf("kmrid%d\n",Lat[pivotListX[pivotChoice]][pivotListY[pivotChoice]]);
      printf("%s\n", "for gnuplot");
