...
int printLattice()
{
   ...
   if ( orientation_type[(K_mer_counter+1)] == h_orientation)          
                  {  //  double r=(double)(rand())/(double)(RAND_MAX);
                //     pivotListX[K_mer_counter] = pivotListX[pivotChoice];
                     headListY[K_mer_counter] = PBCY(pivotListY[K_mer_counter]-(K/2));
                    // printf("\n headListY pbc %d:\n",  headListY[K_mer_counter] );
                     if((headListY[K_mer_counter]+(K-1)) != PBCY(headListY[K_mer_counter]+(K-1))){
                    //    printf("\n kushal_if %f", r);
                        remaining = ((headListY[K_mer_counter]+(K-1))% Lx);
                       // printf("\n K_mer_id %d px: %d py: %d hy: %d 0 %d 1\n",(K_mer_counter+1), pivotListX[K_mer_counter], pivotListY[K_mer_counter], headListY[K_mer_counter], ((K-1) - remaining));
                       // printf("\n K_mer_id %d px: %d py: %d 0 0 %d 1\n",(K_mer_counter+1),  pivotListX[K_mer_counter], pivotListY[K_mer_counter], remaining);
        ...
        //else{     
                    ...
                    // printf("\n headListX pbc: %d\n",  headListX[K_mer_counter]);
                //     pivotListY[K_mer_counter] = pivotListY[pivotChoice];
                     if((headListX[K_mer_counter]+(K-1)) != PBCX(headListX[K_mer_counter]+(K-1))){
                        remaining = ((headListX[K_mer_counter]+(K-1))% Ly);
                       // printf("\n K_mer_id %d hx: %d px: %d py: %d %d 0 2\n",(K_mer_counter+1), headListX[K_mer_counter],pivotListX[K_mer_counter], pivotListY[K_mer_counter],((K-1) - remaining));
                       // printf("\n K_mer_id %d 0 px: %d py: %d %d 0 2\n",(K_mer_counter+1),pivotListX[K_mer_counter], pivotListY[K_mer_counter],remaining);
...
int main (){
    ...
    for(t=0; t<T; t++)
    {
        ...
        if(t % GAP == 0){
            ...
            printf("\ntime_step %d horizontal %d \n vertical %d", t, hor_count, ver_count);
            printf("%s\n", "final");
            printLattice();
        }
    }
